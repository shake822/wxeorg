...
sql = "select s1.goodscode,goodsname,goodsunit,isnull(innum,0) as innum,isnull(inmoney,0) as inmoney,isnull(outnum,0) as outnum,isnull(outmoney,0) as outmoney,isnull(innum-outnum,0) as totalnum,isnull(inmoney-outmoney,0) as totalmoney from (select goodscode,goodsname,goodsunit,isnull(sum(number),0) as innum,isnull(sum(money),0) as inmoney from s_billdetail where billtype = '领料出库 'and AddDate<='"&s_date2&"' and (AddDate)>='"&s_date1&"'"&s_depotname&s_custname&s_goodsname&s_user&" group by goodscode,goodsname,goodsunit) as s1 left join (select goodscode,isnull(sum(number),0) as outnum,isnull(sum(money),0) as outmoney from s_billdetail where billtype = '退料入库 'and (AddDate)<='"&s_date2&"' and (AddDate)>='"&s_date1&"'"&s_depotname&s_custname&s_goodsname&s_user&" group by goodscode) as s2 on s2.goodscode = s1.goodscode"

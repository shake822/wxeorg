...
for i=0 to numi+1
   q3="insert into Dict_Units(c_Depth,Name,c_ChildNum,c_select)values('"&i+1&"','"&request("departname"&i)&"','"&i&"','"&strcode&""&i+1&"')"
...
if err.number=0 then
     conn.commitTrans
	 conn.close
     response.Write "<script>document.location.href='unit.asp'</script>"
     else
	 conn.rollbackTrans
     end if
 end if
...
<form action="<%=action%>" method="post" name="form2" id="form2" >  
          <div id="cgshrk">
            <table class="td_table"   cellspacing="1" cellpadding="0" align="center" rules="all"  width="400" ID="Table5">   
			              <tr>
						   <td colspan="3">
						 <button type="submit" class='button' onClick="getvalue()"><img src="../images/baocun.png" border='0' align='absmiddle'>&nbsp;±£´æ</button>
                          ...
                          <%
                            ...
                            Response.write "<input type=""hidden"" name=""totalMenus"" id=""totalMenus"" value="""&rs.recordcount&""">"  
                                  ...
                                  <tr height=25>   
                                      <td align="center"><%=i+1%><input type="hidden" name="menuorder_<%=i+1%>" id="menuorder_<%=i+1%>" value="<%=i+1%>">
			...
			<button type='button' class='button' onClick="moveMenu('<%=rs("c_ChildNum")%>','<%=i+1%>','up')"><img src="../images/shangyi.png" border='0' align='absmiddle'>&nbsp;ÉÏÒÆ</button>&nbsp;
			<button type='button' class='button' onClick="moveMenu('<%=rs("c_ChildNum")%>','<%=i+1%>','down')"><img src="../images/xiayi.png" border='0' align='absmiddle'>&nbsp;ÏÂÒÆ</button>&nbsp;
			<button type='button' class='button' onClick="getid(<%=i+1%>,<%=rs("c_ChildNum")%>)"><img src="../images/shanchu.png" border='0' align='absmiddle'>&nbsp;É¾³ý</button>  
									<% end if %>
                                       </td>   
                                  </tr>   
                           <% i=i+1  

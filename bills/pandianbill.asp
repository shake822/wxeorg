...
function add_row(){
    ...
    //addrow();
	openwin('../common/selectgoods.asp?depot='+document.getElementById("depot").value+'&type=<%=request("type")%>&adddate='+document.all.date.value+'&id='+j, 800,600)
}
function del_row() {
...
<form method="POST" onKeyPress="if(event.keyCode==13){return false;}" action="../action/savepandian.asp" name="sample">
<br><div align="center" class="style1">盘点单</div>
</font><input type="hidden" id="rowcount" name="rowcount">
<div align="center" class="style1">
<%
today = Now()
tdate = Year(today) & "-" & Right("0" & Month(today), 2) & "-" & Right("0" & Day(today), 2)
sql = "select * from t_bill where billcode like 'PD-"&tdate&"%' order by billcode desc"
Set rs = server.CreateObject("adodb.recordset")
rs.Open sql, conn, 1, 1
If rs.recordcount = 0 Then
    s_billcode = "PD-"&tdate&"-0001"
Else
    s_temp = rs("billcode")
    s_temp = Right(s_temp, 4) + 1
    s_billcode = "PD-"&tdate&"-"&Right("000"&s_temp, 4)
End If
close_rs(rs)
...
<div align="left" class="but_mar">
    <input name="add" type="button" onMouseOver="this.className='button_over';" onMouseOut="this.className='button';"  onClick=add_row() class="button" value=" 添 加 ">
	<input name="del" onclick=del_row() onMouseOver="this.className='button_over';" onMouseOut="this.className='button';"  type="button" class="button" value=" 删 除 ">
